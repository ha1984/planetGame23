...
Region Region1::load_dialog()
{
	...
	region.dialog1.str1 = "Grrrrrr!";
	region.dialog1.str2 = "1) Wer bist du und was ist das für ein Planet hier?";
	region.dialog1.str3 = "2) Ich muss mein Schiff reparieren. Kann ich hier irgendwo Ersatzteile kaufen?";
	
	region.dialog2.str1 = "Bringe mir zunächst einen blauen Kristall als Geschenk, dann sage ich dir wo die Stadt ist.";
	region.dialog2.str2 = "Ok, ich werde mich auf die Suche machen.";
	region.dialog2.str3 = "";

	region.dialog3.str1 = "Kristall aufgenommen.";
	region.dialog3.str2 = "";
	region.dialog3.str3 = "";
	
	region.dialog4.str1 = "Vielen Dank für den Kristall. Die Stadt findest du, wenn du durch das Portal im Norden gehst.";
...
void Region1::setModelToWorlds(struct engine* engine)
{
	engine->renderer->modelToWorlds.resize(numberOfObj);
	engine->renderer->modelToWorlds[0] = translate(mat4(), vec3(0.0, 20.0, 0.0))* scale(mat4(), vec3(90.0f, 90.0f, 90.0f));
	engine->renderer->modelToWorlds[1] = translate(mat4(), vec3(20.0, 5.0, 15.0));
	...
	engine->renderer->modelToWorlds[4] = translate(mat4(),vec3(0.0,0.0,-15.0))*glm::rotate(mat4(), 1.5f, vec3(1.0, 0.0, 0.0))*glm::scale(mat4(),vec3(0.1,0.1,0.1));
...
std::vector<ShapeData> Region1::shape_loader(struct engine* engine, std::vector<int>& csvWidths)
{    
	...
	shapeExternVec[0] = load_geometry(engine, "sky3.txt");
	
	shapeExternVec[1] = load_geometry(engine, "ship2.txt");
	
	shapeExternVec[2] = ShapeGenerator::makePlane(5);
	
	shapeExternVec[3] = load_geometry(engine, "mundus.txt");

	shapeExternVec[4] = load_geometry(engine, "crystal.txt");
...
void Region1::load_Tex(struct engine* engine) {
	...
	asset = AAssetManager_open(manager, "sky3.png", AASSET_MODE_STREAMING);
	...
	asset = AAssetManager_open(manager, "ship2.png", AASSET_MODE_STREAMING);
	...
	asset = AAssetManager_open(manager, "mundus.png", AASSET_MODE_STREAMING);
	fileLength = AAsset_getLength(asset);
	pdata = (unsigned char*)(AAsset_getBuffer(asset));
	load_texture(engine, pdata, 3, false, fileLength);
	AAsset_close(asset);

	asset = AAssetManager_open(manager, "crystal.png", AASSET_MODE_STREAMING);
	fileLength = AAsset_getLength(asset);
	pdata = (unsigned char*)(AAsset_getBuffer(asset));
...
void Region1::png_asset_read(png_structp png, png_bytep data, png_size_t size) {
	AAsset_read(asset, data, size);
}

void Region1::load_texture(struct engine* engine, unsigned char* pdata, GLuint j, bool alpha, off_t fileLength) {
...
std::vector<float> Region1::readCSV(GLuint i, std::vector<int>& csvWidths) {
...
ShapeData Region1::load_geometry(struct engine* engine, const char* filename) {
	AAssetManager* manager = engine->app->activity->assetManager;
